...
if (!Reg.test(text)) return m.reply(`*❌ Incorrect form*\n\nuse this way\nExample: ${prefix}reg name.age`) 
let [_, name, splitter, age] = text.match(Reg)
if (!name) return m.reply('The name cannot be empty') 
if (!age) return m.reply('Age cannot be empty (Numbers)') 
age = parseInt(age)
if (age > 100) return m.reply('Its old (。-`ω´-)') 
if (age < 6) return m.reply('🚼  Based on, babies know how to write.✍️😳') 
if (name.length >= 30) return m.reply('🐈 Fua that based, the name is too long that wants a bridge as a name😹') 
...
if (command == 'unreg') {
const {createHash} = require('crypto') 
if (!args[0]) return m.reply('*✳️ Enter serial number"\n\n*Check your serial number with the command: #myns*') 
const user = global.db.data.users[m.sender];
const sn = createHash('md5').update(m.sender).digest('hex');
if (args[0] !== sn) return m.reply('*⚠️ Incorrect serial number*\n\n*Use:* #myns') 
...
if (new Date - global.db.data.users[m.sender].robs < 600000) return m.reply(`*🚓 He returns ${msToTime(date - new Date())}*`) 
if (m.isGroup) who = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : false;
else who = m.chat;
if (!who) return m.reply(`*⚠️ TAG THE BOBO PERSON*`)
try { 
if (!(who in global.db.data.users)) return m.reply(`*⚠️ The user is not found in my database.*`)
...
m.reply(`*🚓🚓🚓You couldn't rob him because the police protect this user 👮(AFK)*`)}}
...
return m.reply(`╭╌「 *YOUR STATISTICS 🆙* 」
├╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌
├『 Your statistics in real time 🕐 』
├╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌
├─ ❏ *NAME:* ${pushname}
├─ ❏ *XP 🆙:* ${user.exp - min}/${xp}
├─ ❏ *LEVEL:* ${user.level}
