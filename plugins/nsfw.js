...
if (user < 15) throw m.reply(`😐 You are a minor!  go play free fire 😆`) 
if (command == 'hentai') {
...
if (global.db.data.users[m.sender].level < 3) return m.reply(`[ ❇️ ] please upgrade to level 3 .level`) 
...
if (command == 'hentai2') {
if (global.db.data.users[m.sender].level < 3) return m.reply(`[ ❇️ ] please upgrade to upgrade  .level`) 
...
sendImageAsUrl(url, `*Here is my pack 🥵*`)}
