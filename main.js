...
chalk.bold.magenta('\n│────────────\n│⏰SCHEDULE: ') + chalk.magentaBright(moment(t * 1000).tz(place).format('DD/MM/YY HH:mm:ss'),
chalk.bold.red('\n️│🏷️MOD: ') + chalk.bold.white(`[${conn.public ? 'Public' : 'Private'}]`), 
chalk.bold.yellow('\n│📑GUY (SMS): ') + chalk.yellowBright(`${type}`),  
chalk.bold.cyan('\n│📊USERS: ') + chalk.cyanBright(pushname) + ' ➜', gradient.rainbow(userSender), 
m.isGroup ? chalk.bold.greenBright('\n│📤GROUP: ') + chalk.greenBright(groupName) + ' ➜ ' + gradient.rainbow(from) : chalk.bold.greenBright('\n│📥PRIVATE'), 
//chalk.bold.red('\nETIQUETA: ') + chalk.redBright(`[${isBaneed ? 'Banned' : ''}]`),
chalk.bold.white('\n│💬MEASSAGE: ') + chalk.whiteBright(`\n▣────────────···\n${msgs(m.text)}\n`))
)}          
...
let text = [`🦁 ɴᴏᴠᴀʙᴏᴛ - ʜᴀᴠᴇ: ${Object.keys(global.db.data.users).length} users using me I can respond slowly my response speed is: ${latensi.toFixed(4)} 🚀`, `ɴᴏᴠᴀʙᴏᴛ-ᴍᴅ | ᴀᴄᴛɪᴠᴇ ✅️: ${runtime(process.uptime())}\n\ncommand list use #ʟmenu`, `ᴀꜱɪꜰ ᴋɪɴɢ\n${yt}`, `🤖 XoXo-MD use: ${conn.public ? 'Public' : 'Private'} | ✅ Activate: ${runtime(process.uptime())} | 👥 Users: ${Object.keys(global.db.data.users).length}`]
...
m.reply('✳️ This group is not allowed, fake number will be expelled...', m.sender)
conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')}}}
if (global.db.data.chats[m.chat].antiarabe && !isGroupAdmins) {
let forbidPrefixes = ["212", "265", "234", "964", "971", "967", "20", "94", "91"];
for (let prefix of forbidPrefixes) {
if (m.sender.startsWith(prefix)) {
m.reply('✳️ In this group, Arabic numbers are not allowed until next time.a...', m.sender)
conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')}}} 
...
let bang = m.key.id
user = m.sender
conn.sendMessage(m.chat, {text: `\`\`\`「 ANTILINK DETECTED 」\`\`\`\n\n@${user.split("@")[0]} 🤨 That is not allowed rat, you will be expelled from the group....`, mentions: [user], },{quoted: m, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
if (!isBotAdmins) return conn.sendMessage(m.chat, { text: `Save you fucking I'm not admins 🙄\n\n*[ CALL TO ADMINS ]*\n${listAdmin}\n\nThe bot needs admin to eliminate this dirty rat`, mentions: participants.map(a => a.id) }, { quoted: m, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})  
let gclink = (`https://chat.whatsapp.com/`+await conn.groupInviteCode(m.chat))
let isLinkThisGc = new RegExp(gclink, 'i')
let isgclink = isLinkThisGc.test(m.text)
if (isgclink) return reply(`Save you the link sent is from this group`)
if (isGroupAdmins) return reply(`I'll save you because you're an admin: v`)
...
if ((quoted.msg || quoted).seconds > 25) return reply('🤡 Hey Where the hell did you see a sticker that lasts so long stupid 🤣.  (Maximum 15 seconds)')  
media = await quoted.download()  
...
const str = `*「 CONGRATULATIONS LEVEL UP 🆙🥳 」*\n\n🥳 Congratulations @${sender.split`@`[0]} 👏 you leveled up keep it up 👏\n\n*LEVEL :* ${before} ⟿ ${user.level}\n*RANGE :* ${user.role}\n*DATE :* ${new Date().toLocaleString('id-ID')}\n\n_*To know what your top port is, enter the command .lb*_`.trim()
...
m.reply(`*Simsimi api down, disable the ChatBot with:*\n#chatbot off`)
console.log(e)}}
...
await conn.sendMessage(m.chat, {text: `*ʜᴇʟʟᴏ@${sender.split`@`[0]}, It is prohibited to speak to the bots private message, so you will be blocked.*\n*To use the bot join the bot group.*\n\n${nn2}`, mentions: [sender], },{quoted: m}) 
...
const responseMessage = `*𝘚𝘜𝘉𝘉𝘖𝘛𝘚 𝘊𝘖𝘕𝘌𝘊𝘛𝘌𝘋:* ${totalUsers || '0'}\n\n${replyMessage.trim()}`.trim();
...
//Info
case 'menu': case 'help': case 'menucompleto': case 'menu2': case 'audio': case 'new': case 'extreno': case 'rule': menu(m, command, conn, prefix, pushname, sender, pickRandom, fkontak) 
break  
case 'estado': case 'infobot': case 'owner': case 'creador': case 'contacto': case 'grupos': case 'grupoficiales': case 'instalarbot': case 'crearbot': case 'ping': case '923474187615': case '6285787841605': case '61485957144': case 'report': case 'sc': info(command, conn, m, speed, sender, fkontak, pickRandom, pushname, from, msg, text) 
break
...
if (!who) return m.reply(`*⚠️ Tag/mention someone*\n\n*𝘌𝘹𝘦𝘮𝘱𝘭𝘦 :* ${prefix + command} @user`) 
let users = global.db.data.users
users[who].banned = true
m.reply(`*The user was banned and will no longer be able to use my commands.*`)}
break
case 'unbanuser': {
let who 
if (m.isGroup) who = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : false
...
let user = global.db.data.users[who]
if (!who) return m.reply(`*⚠️ Tag/mention the user to unban*`) 
let users = global.db.data.users
users[who].banned = false
m.reply(`*The user has been successfully unbanned and can now use the bot.`)}
break
case 'public': case 'publico': {
if (!isCreator) return reply(info.owner)
conn.public = true
reply('✅Successful change to public use')}
break
case 'self': case 'privado': {
if (!isCreator) return reply(info.owner)
conn.public = false
reply('✅Successful change to private use')}
break	 
case 'autoadmin': case 'tenerpoder': {
if (!m.isGroup) return m.reply(info.group)
if (!isBotAdmins) return m.reply(info.botAdmin)
if (!isCreator) return reply(info.owner)
reply(`${pickRandom(['You are already admin my boss 😎', '*READY YOU ARE NOW ADMIN MY OWNER/DEVELOPED 😎*'])}`)
await conn.groupParticipantsUpdate(m.chat, [m.sender], "promote")}  
break 
case 'leave': {  
if (!isCreator) return reply(info.owner)
reply(m.chat, `*Goodbye, it was a pleasure, you're here see you soon 👋*`)
...
await m.reply('*Sending backup for a moment*')
...
let updatee = execSync('git remote set-url origin https://github.com/Joker-Ofc/XoXo-MD.git && git pull')
await conn.sendMessage(from, { text: updatee.toString() }, { quoted: msg })}  
break
case 'reiniciar': case 'restart': {
if (!isCreator) return reply(info.owner)
m.reply('_🔄 Rebooting Bot..._');
await delay(3 * 3000) 
conn.ws.close()}
break
//__________
...
default:  
if (budy.includes(`All good`)) {
conn.sendPresenceUpdate('composing', m.chat)
await m.reply(`${pickRandom(['Yes friend everything is fine, vite', 'Everything is fine capo and you 😎'])}`)} 
if (budy.includes(`Good morning`)) {
conn.sendPresenceUpdate('composing', m.chat)
m.reply(`${pickRandom(['Good morning shitty trolls', '*Good morning my love 😘*', '*Good morning beautiful morning 🥰*'])}`)}  
if (budy.includes(`Yaoi`)) {
m.reply(`${pickRandom(['What blowjob?  go study better', 'I m a straight bot, dont ask for a blowjob (︶｡︶)zzZ '])}`)}
if (budy.includes(`autodestruction`)) { 
let e = fs.readFileSync('./src/autodestruction.webp')
let or = ['texto', 'sticker'];
let media = or[Math.floor(Math.random() * 2)]
if (media === 'texto')
m.reply('*My boss doesnt like me 😢*') 
if (media === 'sticker')
conn.sendFile(m.chat, e, 'sticker.webp', '',m, true, { contextInfo: { 'forwardingScore': 200, 'isForwarded': false, externalAdReply:{ showAdAttribution: false, title: 'ᶜ self-destruction', mediaType: 2, sourceUrl: nna, thumbnail: imagen1}}}, { quoted: m, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})}
if (budy.includes(`XoXo-MD`)) {
m.react(`${pickRandom(['🌟', '👀', '🤑'])}`)}
...
let media = or[Math.floor(Math.random() * 2)]
if (media === 'sticker')     
conn.sendFile(m.chat, noetiqueta, 'sticker.webp', '',m, true, { contextInfo: { 'forwardingScore': 200, 'isForwarded': false, externalAdReply:{ showAdAttribution: false, title: 'Me what?', mediaType: 2, sourceUrl: nna, thumbnail: imagen1}}}, { quoted: m, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
if (media === 'texto')
await conn.sendMessage(m.chat, {text: `${pickRandom(['*THAT I WHAT?*', 'That?'])}`}, {quoted: m, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})}
...
//--------------------[ REPORTE/ERRORS ]-----------------------     
let e = String(err) 
conn.sendMessage("923474187615@s.whatsapp.net", { text: "Hello Creator/Developer, there seems to be a bug, please fix it 🥲\n\n" + util.format(e), 
contextInfo:{
